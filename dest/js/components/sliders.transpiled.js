...
function toggleDisabledControls(slider, sliderTNS) {
  if (slider.querySelector('.slider__controls')) {
    var sliderTNSinfos = sliderTNS.getInfo();
...
function preventFocusOnSlideOffScreen(slider) {
  slider.querySelectorAll('.tns-item').forEach(function (slide) {
    var slideLeft = parseInt(slide.getBoundingClientRect().left),
    ...
    if (slide.getAttribute('tabindex') == '-1' || slideLeft < sliderLeft || slideRight > sliderRight) {
      slide.querySelectorAll(focusableSelector).forEach(function (elmt) {
...
function initImgAsThumbnail(slider) {
  var imgs = slider.getAttribute('data-slider-img-as-thumbnail').split(',');
  slider.querySelectorAll('.tns-nav button').forEach(function (thumbnail, index) {
    thumbnail.style.backgroundImage = 'url(' + imgs[index] + ')';
  });
}

document.querySelectorAll('.js-slider').forEach(function (slider) {
  var sliderTNS = tns(_objectSpread({
  ...
  }, options[slider.getAttribute('data-slider-options')]));
  slider.querySelectorAll('.js-slider-control').forEach(function (btn) {
  ...
  sliderTNS.events.on('indexChanged', function () {
    toggleDisabledControls(slider, sliderTNS);
  });
  sliderTNS.events.on('transitionEnd', function () {
    preventFocusOnSlideOffScreen(slider);
  });
  sliderTNS.events.on('newBreakpointEnd', function () {
    showHideControls(slider);
    toggleDisabledControls(slider, sliderTNS);
    preventFocusOnSlideOffScreen(slider);
  });
